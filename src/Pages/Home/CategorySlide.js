...
const CategorySlide = () => {
    ...
    return (
        <div className='w-11/12 mx-auto py-14'>
            <h2 className="text-2xl font-poppins font-semibold">Shoping By Category</h2>
                ...
                <Slider {...settings} ref={sliderRef}>
                    {categoryProduct.map((category, index) => (
                        <Link to={`/category/${category._id}`} key={index}>
                            <img
                                className="h-28 w-2/3 mx-auto"
                                src={category?.picture}
                                alt={`Item ${index + 1}`}
                            />
                            <h2 className=" text-black text-2xl font-semibold text-center py-5 ">{category?.name}</h2>
                        </Link>
                    ))}
                </Slider>
                <button
