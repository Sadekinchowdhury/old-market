...
const ShopingCartDetails = ({ handleDecrease, handleIncrease, book, cart }) => {
    ...
    return (
        <div className='w-10/12 mx-auto'>
            ...
            {
                loading && <div className='flex items-center justify-evenly p-4 gap-10'>
                    <div className='flex w-6/12 items-center gap-4'>
                        <Checkbox />
                        <img className='w-20 h-20' src={book?.MobileDetails?.picture} alt="" />
                        <div>
                            <h1 className='text-xl font-semibold'>{book?.MobileDetails?.name}</h1>
                        </div>
                    </div>
                    <div>
                        <h1 className='text-[17px] text-green-700 font-semibold' >${book?.MobileDetails?.price}</h1>
                        <h1 className='line-through text-[15px] font-semibold'>${book?.MobileDetails?.originalprice}</h1>
                        <div>
                            <AiFillDelete onClick={handlDelete} className='hover:text-red-700 cursor-pointer' size={30} />
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        <td>
                            {
                                book?.MobileDetails?.price && !book.MobileDetails?.paid && <Link to={`/dashboard/payment/${book._id}`}><button className='btn btn-primary px-5'>
                                    pay
                                </button></Link>
                            }
                            {
                                book?.MobileDetails?.price && book.MobileDetails?.paid && <button
