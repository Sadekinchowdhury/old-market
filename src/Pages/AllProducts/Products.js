...
const Products = ({ product }) => {
    ...
    return (
        <Link to={`/card_details/${_id}`}
            onMouseEnter={hovemouse}
            onMouseLeave={hovemouse}

            className={`p-4  bg-[#F3F3F3] border-gray-200 hover:bg-white hover:border-gray-200 hover:shadow-2xl transition duration-500 ${open && 'border-gray-300'}`}>
            <div className='py-10 hover:opacity-70'>
                <img className='w-2/3 mx-auto h-36' src={picture} alt="" />
            </div>
            ...
            <div className='pt-10'>
                ...
                <div className='flex py-3 items-center justify-center text-center gap-4'>
                    <h1 className='text-[14px] font-semibold line-through'>${originalprice}</h1>
                    <h1 className='text-[14px] text-green-500 font-semibold '>${price}</h1>
                </div>
            </div>
        </Link>
    );
